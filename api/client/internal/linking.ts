...
async function validateLinkOk(req: ReposAppRequest, res, next) {
  ...
  try {
    ...
    if (block) {
      ...
      return next(jsonError(`This system is not available to guests. You are currently signed in as ${displayName} ${userPrincipalName}. Please sign out or try a private browser window.`, 400));
    }
